...
class MPC
{
    public:
        ...
        vector<double> Solve(Eigen::VectorXd state, Eigen::VectorXd coeffs);//状态 + 因子
        vector<double> mpc_x; //x, y预测轨迹
        vector<double> mpc_y;

        void LoadParams(const std::map<string, double> &params);//load 参数
