...
{
    public:
        MPCNode(); //MPCNode无参构造函数
        int get_thread_numbers(); 
		geometry_msgs::Pose getpose(string ,string); //得到位姿
		void isreached();//判断是否到达
		void pathCycle(nav_msgs::Path *pathMsg);//路径
		bool clear_mappath(nav_msgs::Path *pathMsg, int min_i);//清除地图
        
    private:
        ros::NodeHandle _nh; 
        ros::Subscriber _sub_odom, _sub_path, _sub_goal, _sub_articulate_angle;//接收(odom, path, goal, articulate_angle)
        ros::Publisher _pub_odompath, _pub_polyfitpath, _pub_twist, _pub_mpctraj;//发布(odompath, polyfitpath, twist, mpctraj)
        ros::Timer _timer1;//timer
		ros::Time current_time, after_time, data_using_time; //当前时间，运行后时间， 使用时间
		tf::TransformListener listener;//listener
		//记录数据初始化对象
		...
		Recorder car_path;      //车辆路径
		Recorder reference_path; //参考path

        geometry_msgs::Point _goal_pos; //目标点Pos
        nav_msgs::Odometry _odom; //odom
        nav_msgs::Path _map_path, _mpc_traj,_mpc_polyfitpath, movebase_path; //map_path,mpc_jraj, mpc_polyfitpath, movebase_path
        geometry_msgs::Twist _twist_msg; //twist_msg

        string _globalPath_topic, _goal_topic; //全局路径topic, 目标topic
        string _map_frame, _odom_frame, _car_frame; //map, odom, car坐标系(base_footprint)


        MPC _mpc;//mpc
        map<string, double> _mpc_params;//mpc_参数
...
{
    _goal_pos = goalMsg->pose.position;//位置
    _goal_received = true;//是否收到
    _goal_reached = false;//是否到达
